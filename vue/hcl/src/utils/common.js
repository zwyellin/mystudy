...
export function isArrayLike(v) {
  ...
  return (
    isEqual(type, "array,set,map") ||
    (type === "object" && isValidArrayIndex(v.length))
  );
}
...
export function sortArray(arr = [], path = "", isAsc = true) {
  ...
  return clone(arr).sort((l, r) => {
    ...
    return leftValue < rightValue ? isAsc : leftValue > rightValue ? !isAsc : 0;
  });
}
...
function _separatorNumber(v, separator = ",") {
  ...
  v[0] = v[0]
    ...
    .reduce(
      (total, i, index, arr) =>
        (total +=
          i + (index % 3 === 2 && index !== arr.length - 1 ? separator : "")),
      ""
    )
    .split("")
    .reverse()
...
export function fixedwidth(v, width, char = "0", fillBeforeOrAfter = "before") {
  v = getString(v);
  return fillBeforeOrAfter === "before"
    ? (
        Array(width)
          .fill(char)
          .join("") + v
